...
# true if $1 < $2 by dot-numeric ordering
version_lt() {
...
accept_merge() {
  local pr="$1" current="$2" proposed="$3"
  gh pr comment "$pr" --body "AUTO-MERGE-ACCEPT — policy checks passed (single-file anchor bump $current -> $proposed, verified against live Termux recipe, invariant OK). Merging per exception-only maintainer policy; the build will be triggered by the auto-merge workflow via workflow_call(force=true)." || log "WARN: accept comment failed"
  if [ "$DRY_RUN" = "true" ]; then
    log "PR #$pr ACCEPT verdict (dry-run: not merging)"
    return 0
  fi
  gh pr merge "$pr" --merge || { escalate "$pr" "merge-command-failed"; return 1; }
  log "PR #$pr MERGED (anchor $current -> $proposed)"
  MERGED=1
}
...
for pr in $candidates; do
  ...
  proposed=$(gh pr diff "$pr" | sed -n 's/^+termuxFirefoxVersion *= *"\([0-9][0-9.]*\)".*/\1/p' | head -n1)
  if [ -z "$proposed" ]; then
  ...
  if [ "${#reasons[@]}" -gt 0 ]; then
    ...
    accept_merge "$pr" "$current" "$proposed" || true
  fi
done
